...
module.exports = class FavouriteStore {
  constructor(filePath = path.join(lumine.getConfigDirPath(), "favourite.json")) {
  ...
  load() {
    ...
    try {
      ...
      lumine.notifications.addWarning(`Could not read ${path.basename(this.filePath)}`, {
  ...
  save() {
    try {
      ...
      lumine.notifications.addError(`Could not write ${path.basename(this.filePath)}`, {
        detail: error.message,
        dismissable: true,
      });
    }
