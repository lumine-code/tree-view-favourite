...
module.exports = {
  activate() {
    ...
    this.disposables.add(
      ...
      atom.commands.add("atom-workspace", {
        "tree-view-favourite:edit": () => atom.workspace.open(this.store.filePath),
  ...
  // section header, and pinning a favourite a second time is a no-op.
  addPaths(paths, groupName = atom.config.get("tree-view-favourite.defaultGroup")) {
    let changed = false;
    for (const entryPath of paths) {
      if (!entryPath || entryPath.startsWith(SYNTHETIC_PREFIX)) continue;
      if (this.store.has(entryPath)) continue;
